...
/**
 * Button with a WebGL "specular rim" — a light streak that tracks the
 * pointer around the button's edge. Renders as a router <Link> (`to`),
 * an <a> (`href`), or a <button>.
 *
 * Differences from the reference implementation:
 *  - the render loop only runs while the button is on screen;
 *  - with prefers-reduced-motion, or if WebGL is unavailable, it degrades
 *    to a plain CSS button with a static border.
 */
...
export default function SpecularButton({
  ...
  useEffect(() => {
    ...
    try {
      ...
      // No WebGL: fall back to the CSS border without a re-render.
      btn.classList.add("specular-button--static");
      return;
    }
    ...
    // Only spend frames on buttons that are actually on screen.
  ...
  const tagProps = to ? { to } : href ? { href } : { type, disabled };
  ...
  return (
    <Tag
      ...
      className={`specular-button specular-button--${size}${
        isStatic ? " specular-button--static" : ""
      }${className ? ` ${className}` : ""}`}
      style={{
        "--sb-radius": `${radius}px`,
        "--sb-tint": tint,
